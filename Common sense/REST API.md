...
통합 자원 식별자(Uniform Resource Identifier)는 인터넷에 있는 자원을 나타내는 유일한 주소. 하위개념으로 URL, URN이 있다.

- URL(Uniform Resource Locator): 자원이 어디 있는지를 알려주기 위한 규약.
...
- 애플리케이션 서버가 행동할 판단의 근거가 되는 컨트롤 정보인 API 버전 정보, 응답받고자 하는 MIME 타입[^1] 등은 header 에 담는다.
...
[^1]: 클라이언트에게 전송된 문서의 다양성을 알려주기 위한 메커니즘. ref) https://developer.mozilla.org/ko/docs/Web/HTTP/Basics_of_HTTP/MIME_types